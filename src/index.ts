            ...
            if (resource.attributes[resourceKey] !== 'undefined' && resource.attributes[resourceKey] !== null) {
                ...
                rids.forEach((rid, idx) => {
                    ...
                    const promise = RelatedCls.detail(rid).then((instance) => {
                        assert(
                            instance instanceof RelatedCls,
                            `Related class detail() returned invalid instance on key ${this.name}.related.${resourceKey}: ${instance.getConstructor().name} (returned) !== ${RelatedCls.name} (related)`
                        )
                        let thisResourceList: ResourceLike[] = <ResourceLike[]>resource.related[resourceKey]
                        if (isMany) {
